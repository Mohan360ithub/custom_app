...
class STimesheet(Document):
	...
	def update_task_and_project(self):
		...
		for data in self.time_logs:
			if data.task and data.task not in tasks:
				task = frappe.get_doc("S Task", data.task)
				task.update_time_and_costing()
				task.save()
				tasks.append(data.task)

			elif data.project and data.project not in projects:
				frappe.get_doc("S Project", data.project).update_project()
	...
	def set_project(self, data):
		data.project = data.project or frappe.db.get_value("S Task", data.task, "project")
	...
	def get_overlap_for(self, fieldname, args, value):
		timesheet = frappe.qb.DocType("S Timesheet")
		timelog = frappe.qb.DocType("S Timesheet Detail")
...
def get_projectwise_timesheet_data(project=None, parent=None, from_time=None, to_time=None):
	...
	query = f"""
		...
		FROM `tabS Timesheet Detail` tsd
...
def get_timesheet_detail_rate(timelog, currency):
	timelog_detail = frappe.db.sql(
		...
		ts.currency as currency FROM `tabS Timesheet Detail` tsd
...
def get_timesheet(doctype, txt, searchfield, start, page_len, filters):
	...
	return frappe.db.sql(
		"""select distinct tsd.parent from `tabS Timesheet Detail` tsd,
			`tabS Timesheet` ts where
...
def make_sales_invoice(source_name, item_code=None, customer=None, currency=None):
	...
	timesheet = frappe.get_doc("S Timesheet", source_name)
...
def get_events(start, end, filters=None):
	...
	return frappe.db.sql(
		"""select `tabS Timesheet Detail`.name as name,
			`tabS Timesheet Detail`.docstatus as status, `tabS Timesheet Detail`.parent as parent,
			from_time as start_date, hours, activity_type,
			`tabS Timesheet Detail`.project, to_time as end_date,
			CONCAT(`tabS Timesheet Detail`.parent, ' (', ROUND(hours,2),' hrs)') as title
		from `tabS Timesheet Detail`, `tabTimesheet`
		where `tabS Timesheet Detail`.parent = `tabTimesheet`.name
...
def get_timesheets_list(
	...
	if customer:
		...
		timesheets = frappe.db.sql(
			...
			FROM `tabTimesheet` ts, `tabS Timesheet Detail` tsd
