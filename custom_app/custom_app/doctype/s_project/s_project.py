...
class SProject(Document):
	...
	def onload(self):
		self.set_onload(
			...
			from `tabS Timesheet Detail` where project=%s and docstatus < 2 group by activity_type
	...
	def copy_from_template(self):
		...
		if self.project_template and not frappe.db.get_all("Task", dict(project=self.name), limit=1):
			...
			for task in template.tasks:
				template_task_details = frappe.get_doc("S Task", task.task)
	...
	def dependency_mapping(self, template_tasks, project_tasks):
		for project_task in project_tasks:
			template_task = frappe.get_doc("S Task", project_task.template_task)
	...
	def update_percent_complete(self):
		...
		total = frappe.db.count("S Task", dict(project=self.name))
		...
		if not total:
			...
			if (self.percent_complete_method == "Task Completion" and total > 0) or (
				...
				completed = frappe.db.sql(
					"""select count(name) from `tabS Task` where
			...
			if self.percent_complete_method == "Task Progress" and total > 0:
				progress = frappe.db.sql(
					"""select sum(progress) from `tabS Task` where
					project=%s""",
					self.name,
				)[0][0]
			...
			if self.percent_complete_method == "Task Weight" and total > 0:
				weight_sum = frappe.db.sql(
					"""select sum(task_weight) from `tabS Task` where
					project=%s""",
					self.name,
				)[0][0]
				weighted_progress = frappe.db.sql(
					"""select progress, task_weight from `tabS Task` where
					project=%s""",
					self.name,
					as_dict=1,
				)
	...
	def update_costing(self):
		...
		TimesheetDetail = frappe.qb.DocType("S Timesheet Detail")
		from_time_sheet = (
...
def get_timeline_data(doctype: str, name: str) -> dict[int, int]:
	...
	timesheet_detail = frappe.qb.DocType("S Timesheet Detail")
...
def create_duplicate_project(prev_doc, project_name):
	...
	task_list = frappe.get_all("S Task", filters={"project": prev_doc.get("name")}, fields=["name"])

	# Create duplicate task for all the task
	for task in task_list:
		task = frappe.get_doc("S Task", task)
...
def send_project_update_email_to_users(project):
	doc = frappe.get_doc("S Project", project)
...
def update_project_sales_billing():
	...
	exists_query = (
		"(SELECT 1 from `tab{doctype}` where docstatus = 1 and project = `tabS Project`.name)"
	)
	project_map = {}
...
def create_kanban_board_if_not_exists(project):
	...
	project = frappe.get_doc("S Project", project)
	if not frappe.db.exists("Kanban Board", project.project_name):
		quick_kanban_board("S Task", project.project_name, "status", project.name)
...
def set_project_status(project, status):
	...
	for task in frappe.get_all("S Task", dict(project=project.name)):
		frappe.db.set_value("S Task", task.name, "status", status)
